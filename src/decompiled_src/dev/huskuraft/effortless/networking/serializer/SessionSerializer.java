...
import dev.huskuraft.effortless.networking.serializer.ModSerializer;
import dev.huskuraft.effortless.session.Session;

public class SessionSerializer
implements NetByteBufSerializer<Session> {
    @Override
    public Session read(NetByteBuf byteBuf) {
        return new Session(byteBuf.readEnum(LoaderType.class), byteBuf.readString(), byteBuf.readString(), byteBuf.readList(new ModSerializer()), byteBuf.readInt());
    }
