...
import dev.huskuraft.effortless.networking.serializer.BlockInteractionSerializer;
import dev.huskuraft.effortless.networking.serializer.ConstraintConfigSerializer;
import dev.huskuraft.effortless.networking.serializer.SnapshotSerializer;
import dev.huskuraft.effortless.networking.serializer.StructureSerializer;
...
implements NetByteBufSerializer<Context> {
    @Override
    public Context read(NetByteBuf byteBuf) {
        return new Context(byteBuf.readUUID(), byteBuf.readEnum(BuildState.class), byteBuf.readEnum(BuildType.class), new Context.Interactions(byteBuf.readList(buffer1 -> buffer1.readNullable(new BlockInteractionSerializer()))), byteBuf.read(new StructureSerializer()), byteBuf.read(new ClipboardSerializer()), byteBuf.read(new PatternSerializer()), byteBuf.read(new ReplaceSerializer()), new Context.Configs(byteBuf.read(new ConstraintConfigSerializer()), byteBuf.read(new BuilderConfigSerializer())), byteBuf.readNullable(new ExtrasSerializer()));
    }
    ...
    public static class ReplaceSerializer
    ...
    public static class BuilderConfigSerializer
    ...
    implements NetByteBufSerializer<Context.Extras> {
        ...
        public Context.Extras read(NetByteBuf byteBuf) {
            return new Context.Extras(byteBuf.readResourceLocation(), byteBuf.read(new EntityStateSerializer()), byteBuf.readEnum(GameMode.class), byteBuf.readLong(), byteBuf.read(new InventorySnapshotSerializer()));
        }
    ...
    public static class InventorySnapshotSerializer
