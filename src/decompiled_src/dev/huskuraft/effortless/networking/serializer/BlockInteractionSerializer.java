...
import dev.huskuraft.effortless.networking.serializer.BlockPositionSerializer;
import dev.huskuraft.effortless.networking.serializer.Vector3dSerializer;

public class BlockInteractionSerializer
implements NetByteBufSerializer<BlockInteraction> {
    @Override
    public BlockInteraction read(NetByteBuf byteBuf) {
        return new BlockInteraction(byteBuf.read(new Vector3dSerializer()), byteBuf.readEnum(Direction.class), byteBuf.read(new BlockPositionSerializer()), byteBuf.readBoolean());
    }
